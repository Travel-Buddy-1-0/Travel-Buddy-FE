...
const handleSubmit = () => {
  ...
  // ✅ Lấy province đầy đủ
  const selectedProvince = provinces.find((p) => p.code === destination);

  // ✅ Lấy đầy đủ activity object (name + iconName)
  ...
  // ✅ Lưu packageData với cả location
  const packageData = {
    destination: selectedProvince ? selectedProvince.name : "",
    locationCode: selectedProvince ? selectedProvince.code : "", // thêm code
  ...
  // ⏳ Sau 3 giây tắt success và về trang chủ
  setTimeout(() => {
    setSuccess(false);
    navigate("/");
  }, 3000);
          ...
          {!success ? (
            <>
              ...
              <p className="text-gray-700 font-medium mb-3">
                Sở thích hoạt động{" "}
                <span className="text-gray-400">(tối đa 5 mục)</span>
              </p>
